...
void DrawSkillList()
{
	...
	for (i = 0; i < NUM_INVLOC; i++) {
		...
		if (pi->_itype != ITYPE_NONE/* && pi->_iCharges > 0*/ && pi->_iSpell != SPL_NULL && pi->_iStatFlag /*&& plr._pMagic >= spelldata[pi->_iSpell].sReqMag*/) {
	...
	numPlrSkills = SPLROWICONLS;
	{
		...
		for (i = 0; mask != 0 && i < NUM_SPELLS; mask >>= 1, i++) {
			if (!(mask & 1)) {
				continue;
			}

			const PlrSkillUse listSkill = { (BYTE)(i + 1), SPLFROM_MANA };
