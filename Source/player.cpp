...
void InitPlayerGFX(int pnum)
{
	...
	if (players[pnum]._pHitPoints < (1 << 6)) {
		...
		else if (!(players[pnum]._pSkillFlags & SFLAG_BLOCK))
			gfxflag &= ~PFILE_BLOCK;
	}
...
static void StartWalk1(int pnum, int xvel, int yvel, int xadd, int yadd)
{
	...
	p->_pfutx = /*p->_pVar1 =*/ px; // the Player's x-coordinate after the movement
	p->_pfuty = /*p->_pVar2 =*/ py; // the Player's y-coordinate after the movement
...
static void StartWalk2(int pnum, int xvel, int yvel, int xoff, int yoff, int xadd, int yadd)
{
	...
	dPlayer[px][py] = -(pnum + 1);
	//p->_pVar1 = px;  // the starting x-coordinate of the player
	//p->_pVar2 = py;  // the starting y-coordinate of the player
	px += xadd;
	py += yadd;
	p->_px = p->_pfutx = px; // Move player to the next tile to maintain correct render order
	p->_py = p->_pfuty = py;
	dPlayer[px][py] = pnum + 1;
	//if (p->_plid != -1) {
...
static bool StartWalk(int pnum)
{
	...
	NewPlrAnim(pnum, p->_pWAnim, dir, p->_pWFrames, PlrAnimFrameLens[PA_WALK], p->_pWWidth);

	if (pnum == myplr) {
		ScrollInfo._sdx = p->_poldx - ViewX;
		ScrollInfo._sdy = p->_poldy - ViewY;
...
void StartNewLvl(int pnum, int fom, int lvl)
{
	...
	case WM_DIABTWARPUP:
		if (pnum == myplr)
			players[pnum].pTownWarps |= 1 << (currLvl._dType - 2);
		players[pnum].plrlevel = lvl;
		break;
	default:
...
static bool WeaponDur(int pnum, int durrnd)
{
	...
	// check dual-wield
	pi = &players[pnum].InvBody[INVLOC_HAND_RIGHT];
	...
	pi = &players[pnum].InvBody[INVLOC_HAND_LEFT];
...
static bool PlrTryHit(int pnum, int sn, int sl, int dx, int dy)
{
	...
	mpo = dPlayer[dx][dy];
	if (mpo != 0 && players[myplr]._pTeam != players[mpo]._pTeam) {
		mpo = mpo >= 0 ? mpo - 1 : -(mpo + 1);
...
static void ArmorDur(int pnum)
{
	...
	loc = INVLOC_CHEST;
	pi = &players[pnum].InvBody[INVLOC_CHEST];
	pio = &players[pnum].InvBody[INVLOC_HEAD];
	if (pi->_itype == ITYPE_NONE) {
...
static void CheckNewPath(int pnum)
{
	...
	if (p->destAction == ACTION_ATTACKMON) {
		...
		MakePlrPath(pnum, players[p->destParam1]._pfutx, players[p->destParam1]._pfuty, false);
	}
...
static void ValidatePlayer()
{
	...
	if ((unsigned)myplr >= MAX_PLRS) {
		dev_fatal("ValidatePlayer: illegal player %d", myplr);
	}
	p = &players[myplr];
...
void ProcessPlayers()
{
	...
	if ((unsigned)myplr >= MAX_PLRS) {
		dev_fatal("ProcessPlayers: illegal player %d", myplr);
	}
	...
	for (pnum = 0; pnum < MAX_PLRS; pnum++) {
		if (players[pnum].plractive && currLvl._dLevelIdx == players[pnum].plrlevel && (pnum == myplr || !players[pnum]._pLvlChanging)) {
			CheckCheatStats(pnum);

			if (pnum == myplr) {
				//if (!PlrDeathModeOK(pnum) && players[pnum]._pHitPoints < (1 << 6)) {
				if (players[pnum]._pHitPoints < (1 << 6) && !players[pnum]._pInvincible) {
					SyncPlrKill(pnum, DMGTYPE_UNKNOWN);
				}
				if ((players[pnum]._pIFlags & ISPL_DRAINLIFE) && currLvl._dLevelIdx != DLV_TOWN && !players[pnum]._pInvincible) {
					PlrDecHp(pnum, 4, DMGTYPE_NPC);
				}
				if (players[pnum]._pIFlags & ISPL_NOMANA && players[pnum]._pMana > 0) {
					PlrSetMana(pnum, 0);
				}
			}
			...
			if (players[pnum]._pAnimCnt >= players[pnum]._pAnimFrameLen) {
...
void ClrPlrPath(int pnum)
{
	...
	memset(players[pnum].walkpath, DIR_NONE, sizeof(players[pnum].walkpath));
}
...
bool PosOkPlayer(int pnum, int x, int y)
{
	...
	if (IN_DUNGEON_AREA(x, y) && /*dPiece[x][y] != 0 &&*/ !nSolidTable[dPiece[x][y]]) {
		...
		if (mpo != 0) {
			mpo = mpo >= 0 ? mpo - 1 : -(mpo + 1);
			if (mpo != pnum && players[mpo]._pHitPoints >= (1 << 6)) {
				return false;
			}
		}
		mpo = dMonster[x][y];
...
void MakePlrPath(int pnum, int xx, int yy, bool endspace)
{
	...
	players[pnum].walkpath[path] = DIR_NONE;
}
...
{
	assert(players[pnum].plrlevel == currLvl._dLevelIdx);
...
{
	if ((unsigned)myplr >= MAX_PLRS) {
		dev_fatal("InitDungMsgs: illegal player %d", myplr);
	}

	players[myplr].pDungMsgs = 0;
#ifdef HELLFIRE
	players[myplr].pDungMsgs2 = 0;
#endif
}
...
void PlayDungMsgs()
{
	...
	if ((unsigned)myplr >= MAX_PLRS) {
		dev_fatal("PlayDungMsgs: illegal player %d", myplr);
	}
	p = &players[myplr];
	...
	} else if (currLvl._dLevelIdx == DLV_CATACOMBS1 && !p->_pLvlVisited[DLV_CATACOMBS1] && !(p->pDungMsgs & DMSG_CATACOMBS)) {
		p->pDungMsgs |= DMSG_CATACOMBS;
		sfxdelay = 40;
		sfxdnum = TEXT_DM_CATACOMBS;
	} else if (currLvl._dLevelIdx == DLV_CAVES1 && !p->_pLvlVisited[DLV_CAVES1] && !(p->pDungMsgs & DMSG_CAVES)) {
		p->pDungMsgs |= DMSG_CAVES;
		sfxdelay = 40;
		sfxdnum = TEXT_DM_CAVES;
	} else if (currLvl._dLevelIdx == DLV_HELL1 && !p->_pLvlVisited[DLV_HELL1] && !(p->pDungMsgs & DMSG_HELL)) {
		p->pDungMsgs |= DMSG_HELL;
		sfxdelay = 40;
		sfxdnum = TEXT_DM_HELL;
	} else if (currLvl._dLevelIdx == DLV_HELL4 && !p->_pLvlVisited[DLV_HELL4] && !(p->pDungMsgs & DMSG_DIABLO)) { // BUGFIX: _pLvlVisited should check 16 or this message will never play
		p->pDungMsgs |= DMSG_DIABLO;
		sfxdelay = 40;
		sfxdnum = TEXT_DM_DIABLO;
#ifdef HELLFIRE
	} else if (currLvl._dLevelIdx == DLV_NEST1 && !p->_pLvlVisited[DLV_NEST1] && !(p->pDungMsgs2 & DMSG2_DEFILER)) {
		p->pDungMsgs2 |= DMSG2_DEFILER;
		sfxdelay = 10;
		sfxdnum = TEXT_DM_NEST;
	} else if (currLvl._dLevelIdx == DLV_NEST3 && !p->_pLvlVisited[DLV_NEST3] && !(p->pDungMsgs2 & DMSG2_DEFILER1)) {
		p->pDungMsgs2 |= DMSG2_DEFILER1;
		sfxdelay = 10;
		sfxdnum = TEXT_DM_DEFILER;
	} else if (currLvl._dLevelIdx == DLV_CRYPT1 && !p->_pLvlVisited[DLV_CRYPT1] && !(p->pDungMsgs2 & DMSG2_DEFILER2)) {
		p->pDungMsgs2 |= DMSG2_DEFILER2;
		sfxdelay = 30;
		sfxdnum = TEXT_DM_CRYPT;
#endif
	} else if (currLvl._dLevelIdx == SL_SKELKING && !p->_pLvlVisited[SL_SKELKING] && !(p->pDungMsgs & DMSG_SKING)) {
