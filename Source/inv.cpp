...
static bool AutoPlace(int pnum, int ii, int sx, int sy, ItemStruct *is)
{
	...
	if (done && is != NULL) {
		NetSendCmdChItem(is, INVITEM_INV_FIRST + p->_pNumInv);
	...
	for (i = 0; i < MAXBELTITEMS; i++) {
		if (players[pnum].SpdList[i]._itype == ITYPE_NONE) {
			if (saveflag) {
				...
				copy_pod(players[pnum].SpdList[i], *is);
				CalcPlrScrolls(pnum);
...
void SyncPlrItemChange(int pnum, BYTE bLoc, int ii)
{
	...
	if (bLoc < INVITEM_INV_FIRST) {
		// body item
		copy_pod(players[pnum].InvBody[bLoc], *is);
		CalcPlrInv(pnum, true);
		return;
	}
	ItemStatOk(pnum, is);
	if (bLoc < INVITEM_BELT_FIRST) {
		...
		copy_pod(players[pnum].InvList[bLoc], *is);
		if (bLoc >= players[pnum]._pNumInv) {
			// place a new item
			assert(bLoc == players[pnum]._pNumInv);
		...
		copy_pod(players[pnum].SpdList[bLoc], *is);
		CalcPlrScrolls(pnum);
	}
}
...
void SyncPlrItemRemove(int pnum, BYTE bLoc)
{
	...
	if (bLoc < INVITEM_INV_FIRST) {
		// body item
		p->InvBody[bLoc]._itype = ITYPE_NONE;
		CalcPlrInv(pnum, p->_pmode != PM_DEATH);
	} else if (bLoc < INVITEM_BELT_FIRST) {
		// inv item
		bLoc -= INVITEM_INV_FIRST;
		p->_pNumInv--;
		if (bLoc != p->_pNumInv)
			copy_pod(p->InvList[bLoc], p->InvList[p->_pNumInv]);
		CalcPlrScrolls(pnum);
	} else {
		// belt item
		bLoc -= INVITEM_BELT_FIRST;
		p->SpdList[bLoc]._itype = ITYPE_NONE;
		CalcPlrScrolls(pnum);
	}
}
...
bool InvUseItem(int cii)
{
	...
	if (cii <= INVITEM_INV_LAST) {
		...
		is = &players[pnum].InvList[iv];
		speedlist = false;
	} else {
		iv = cii - INVITEM_BELT_FIRST;
		is = &players[pnum].SpdList[iv];
		speedlist = true;
	}
	...
	if (!is->_iStatFlag) {
		PlaySFX(sgSFXSets[SFXS_PLR_13][players[pnum]._pClass]);
		return true;
	}
	...
	case IMISC_RUNE:
		...
		if (spelldata[sn].scCurs != CURSOR_NONE) {
			...
			players[pnum]._pTSpell = sn;
			players[pnum]._pTSplFrom = cii;
		} else {
...
void SyncUseItem(int pnum, int cii)
{
	...
	if (AllLevels[players[pnum].plrlevel].dType == DTYPE_TOWN
#ifdef HELLFIRE
	 && (is->_iMiscId == IMISC_SCROLL || is->_iMiscId == IMISC_RUNE)
#else
	 && is->_iMiscId == IMISC_SCROLL
	...
	case IMISC_BOOK:
		sn = is->_iSpell;
		players[pnum]._pMemSkills |= SPELL_MASK(sn);
		PlrIncMana(pnum, spelldata[sn].sManaCost << 6);
		// CalcPlrSpells(pnum);
		break;
	case IMISC_SPECELIX:
