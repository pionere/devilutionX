...
static void switch_keyboard_get(const char* guide_text, char* initial_text, int max_len, char* buf)
{
	Result rc = 0;
...
void switch_start_text_input(const char* guide_text, char* initial_text, int max_length)
{
	char text[max_length + 1] = { '\0' };
	switch_keyboard_get(guide_text, initial_text, max_length, text);
	for (int i = 0; i < 600; i++) {
	...
	if (text[0] == '\0') {
		strncpy(text, initial_text, max_length);
		text[max_length] = { '\0' };
	}
	char* cursor = text;
